import {Tile, Tag} from "@triss/entities";
...
export class TwoDArray<K> {
  ...
  static createFromEntities<K extends Tile | Tag>(entities: K[]): TwoDArray<K> {
    let maxY = 0;
    let maxX = 0;
    ...
    const instance = new TwoDArray<K>(maxX + 1, maxY + 1);
      ...
      if (instance.has(x, y))
        throw new Error(
          "The entity with the id " +
            entity.id +
            " and the entity with the id " +
            instance.get(x, y)!.id +
            " have the same position."
        );

      instance.set(x, y, entity);
    }
